    ...
    <ion-footer v-if="currentOrder.statusId === 'ORDER_APPROVED' && selectedSegment === 'open'">
      <ion-toolbar>
        <ion-buttons slot="end">
          <ion-button color="dark" fill="outline" :disabled="!hasPermission(Actions.APP_TRANSFER_ORDER_CANCEL) || isCreatingShipment" @click="closeTOItems()">
            {{ translate("Close Items") }}
          </ion-button>
          <ion-button v-show="areItemsEligibleForRejection" color="danger" fill="outline" :disabled="!hasPermission(Actions.APP_TRANSFER_ORDER_CANCEL) || isCreatingShipment" @click="rejectItems()">
