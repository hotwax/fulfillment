...
const fetchCarrierManifestInformation = async (carrierIds: Array<string>) => {
  for (const partyId of carrierIds) {
    // Manifests generated by OMS (legacy FacilityContent flow)
    const payload = {
      customParametersMap: {
        partyId,
    ...
    // Manifests generated through Unigate (CarrierManifest records)
    try {
      const resp = await carrierStore.fetchCarrierManifests({
        facilityId: currentFacility.value.facilityId,
        carrierPartyId: partyId,
        statusId: "MANIFEST_GENERATED",
        transmitDate_from: DateTime.now().startOf("day").minus({ days: 7 }).toMillis(),
        orderByField: "-transmitDate",
        pageIndex: 0,
        pageSize: 250
      });

      if (!commonUtil.hasError(resp) && resp.data?.length) {
        const unigateManifests = resp.data.map((manifest: any) => ({
          manifestId: manifest.manifestId,
          manifestUrl: manifest.manifestUrl,
          facilityId: manifest.facilityId,
          fromDate: manifest.transmitDate
        }));

        if (carrierConfiguration.value[partyId]) {
          carrierConfiguration.value[partyId]["manifests"] = unigateManifests;
        } else {
          carrierConfiguration.value[partyId] = { manifests: unigateManifests };
        }
      }
