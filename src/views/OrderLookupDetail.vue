            ...
            <div>
              ...
              <ion-card>
                ...
                <div v-if="order.orderPayments?.length">
                  <ion-list data-testid="order-lookup-detail-payment-info" v-for="(orderPayment, index) in order.orderPayments" :key="index">
                    <ion-item lines="none">
        ...
        <section>
          <div v-if="order.shipGroups && order.shipGroups.length">
            <div v-for="shipGroup in order.shipGroups as Array<any>" :key="shipGroup.shipGroupSeqId" class="ship-group-info ion-margin-vertical">
              <ion-item lines="none" data-testid="order-lookup-detail-ship-group">
                ...
                <ion-button data-testid="order-lookup-detail-tracking-actions" slot="end" fill="clear" color="medium" @click="shippingLabelActionPopover($event, shipGroup)" v-if="shipGroup.trackingIdNumber">
              ...
              <div class="product-card">
                <ion-card v-for="item in shipGroup.items" :key="item.orderItemSeqId">
                  ...
                  <ion-item v-if="item.statusId !== 'ITEM_CANCELLED' && item.statusId !== 'ITEM_COMPLETED'">
                    ...
                    <ion-button data-testid="order-lookup-detail-fetch-stock-button" v-else fill="clear" @click.stop="fetchProductStock(item.productId, shipGroup.facilityId)">
