...
import { fetchProducts, isError, Product } from "@hotwax/oms-api";
...
const actions: ActionTree<ProductState, RootState> = {
  ...
  async findProduct ({ commit, state }, payload) {
    ...
    try {
      ...
      // resp.total tells the number of items in the response
      if (!isError(resp) && resp.total > 0) {
        let products: Array<Product> = resp.products;
        const totalProductsCount: number = resp.total;
