...
This repository includes a robust Playwright End-to-End (E2E) testing suite capable of testing multiple client environments (e.g., UAT instances) dynamically. The testing framework automatically provisions projects for each client defined in your `.env` file.

### Environment Setup

Ensure your `.env` file has a valid `CLIENTS` JSON block containing credentials for the environments you want to test (e.g., `krewe-uat`, `thirdlove-uat`, `adoc-gt-uat`).

### Running Tests

**1. Run All Tests Across All Clients:**
This will execute the entire test suite against *every* client configured in your environment.
```bash
npx playwright test
```

**2. Run Tests for a Specific Client (Recommended):**
You can target a specific client environment by specifying the project name. The project name format is usually `chromium-<client-id>`.

*Example: Running tests for Krewe UAT:*
```bash
npx playwright test --project=chromium-krewe-uat
```

*Example: Running tests for ThirdLove UAT:*
```bash
npx playwright test --project=chromium-thirdlove-uat
```

**3. Using the `CLIENT` Environment Variable Filter:**
Alternatively, you can filter which projects are generated by Playwright at runtime using the `CLIENT` environment variable. This allows you to run multiple specific clients at once using a comma-separated list.

*Example: Run Krewe and ThirdLove only:*
```bash
CLIENT="krewe-uat,thirdlove-uat" npx playwright test
```

### Specific Scenarios & Combinations

**Running a specific test file:**
```bash
npx playwright test Tests/e2e/specs/negative-flows/create-transfer-order-breaking.spec.js --project=chromium-krewe-uat
```

**Running tests in headed mode (Visually watch the browser):**
```bash
HEADED=true npx playwright test --project=chromium-krewe-uat
```

**Running tests with the UI Dashboard:**
```bash
npx playwright test --ui
```

**Viewing HTML Reports:**
If a test fails, you can view the detailed HTML report (which includes traces, DOM snapshots, and network logs):
```bash
