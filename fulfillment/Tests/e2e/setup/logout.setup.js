...
async function performLogin(page, config) {
  ...
  // Wait for the URL to contain fulfillment or wait for network idle to ensure redirection is complete
...
setup("authenticate and save storage state", async ({ page }, testInfo) => {
  ...
  const clientId = projectName.replace("setup-logout-", "");
